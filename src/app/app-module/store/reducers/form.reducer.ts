...
const setValue = (obj, prop, val) => {
  ...
  split.reduce((acc, part) => {
    if (part === last) {
      if (['dirty', 'status', 'errors'].indexOf(part) === -1) {
        acc[part] = { value: val[part] };
      } else {
        acc[part] = val;
      }
    } else {
...
export function form(reducer: Function) {
  return function(state: any, action: any) {
    ...
    if (
      action.type === FormActions.UpdateValue ||
      action.type === FormActions.UpdateForm
    ) {
      nextState = setValue(nextState, `${action.payload.path}`, {
        ...action.payload.value,
      });
    }

    if (
      action.type === FormActions.UpdateStatus ||
      action.type === FormActions.UpdateForm
    ) {
      nextState = setValue(
        nextState,
    ...
    if (
      action.type === FormActions.UpdateErrors ||
      action.type === FormActions.UpdateForm
    ) {
    ...
    if (
      action.type === FormActions.UpdateDirty ||
      action.type === FormActions.UpdateForm
    ) {
      nextState = setValue(
        nextState,
